...
entity loadstore3_tb is
end loadstore3_tb;

architecture bhv of loadstore3_tb is
...
process begin
    ...
    assert user_reg(1) = x"ffffeeff" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 14 * CLK_PERIOD;
    assert user_reg(1) = x"0000eeff" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 16 * CLK_PERIOD;
    assert user_reg(1) = x"00001122" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 18 * CLK_PERIOD;
    assert user_reg(1) = x"eeff1122" severity FAILURE;
    wait;
end process;
    end block assertBlk;
end bhv;
