...
architecture bhv of arith5_tb is
    component cpu
        port (
...
alias user_hi is <<signal ^.cpu_inst.hi_lo_ist.hiData: std_logic_vector(DataWidth)>>;
alias user_lo is <<signal ^.cpu_inst.hi_lo_ist.loData: std_logic_vector(DataWidth)>>;
    begin
        -- CODE BELOW IS AUTOMATICALLY GENERATED
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 8 * CLK_PERIOD;
    assert user_lo = 32ux"0" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 9 * CLK_PERIOD;
    assert user_lo = 32ux"f9d9ffa0" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 9 * CLK_PERIOD;
    assert user_hi = 32ux"ffffffff" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 12 * CLK_PERIOD;
    assert user_reg(4) = 32ux"ffff5678" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
...
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 14 * CLK_PERIOD;
    assert user_lo = 32ux"c096d7e0" severity FAILURE;
    wait;
end process;
    end block assertBlk;
end bhv;
