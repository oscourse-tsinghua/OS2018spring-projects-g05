...
architecture bhv of cache1_tb is
...
begin
    ram_ist: entity work.cache1_fake_ram
        port map (
            clk => clk,
            rst => rst,
            enable_i => devEnable,
...
process begin
    ...
    assert user_reg(3) = 32x"00001234" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 14 * CLK_PERIOD;
    assert user_reg(9) = 32x"00000000" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 15 * CLK_PERIOD;
    assert user_reg(9) = 32x"0000ffff" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 19 * CLK_PERIOD;
    assert user_reg(8) = 32x"00000000" severity FAILURE;
    wait;
end process;
process begin
    wait for CLK_PERIOD; -- resetting
    wait for 20 * CLK_PERIOD;
    assert user_reg(8) = 32x"0000ffff" severity FAILURE;
    wait;
end process;
    end block assertBlk;
end bhv;
