...
各阶段寄存器的值在时钟上升沿发生如下变化：若流水线需要清空，或是ctrl实体恰好暂停了译码阶段，却没有暂停后续的执行阶段，则置零/无效信号，因为没有指令可供执行阶段执行；否则，若译码和执行阶段均被暂停，则维持上一个周期的值，以便译码执行重复执行；再否则，置为来自取指译码的值，让执行阶段获得新指令。
...
\hline isInDelaySlot\_i        & 1                      & I     & id\_ex        & 高电平表示本指令在延迟槽内 \\
\hline toStall\_o              & 1                      & O     & ctrl          & 请求暂停 \\
\hline toWriteReg\_o           & 1                      & O     & ex\_mem       & 译码结果：通用寄存器写使能 \\
\hline writeRegAddr\_o         & RegAddrWidth           & O     & ex\_mem       & 译码结果：通用寄存器写地址 \\
\hline writeRegData\_o         & DataWidth              & O     & ex\_mem       & 执行结果：通用寄存器写数据 \\
...
\hline toWriteHi\_i            & DataWidth              & O     & ex\_mem       & 执行结果：写入HI寄存器的值 \\
\hline toWriteLo\_i            & DataWidth              & O     & ex\_mem       & 执行结果：写入LO寄存器的值 \\
